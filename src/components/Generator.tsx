...
const Generator: React.FC = () => {
  ...
  const { setPaymentParams } = useContext(ParamsContext);
  ...
  const updateParam = (key: keyof GeneratorParams, value: any) => {
          ...
          {sliderConfigs.map(config => (
            <div key={config.id} className="bg-gray-900 border border-red-600 rounded-lg p-6 shadow-xl">
              ...
              <input
                ...
                max={config.max}
                step={config.step}
                value={params[config.id] as number}
                onChange={(e) => updateParam(config.id, Number(e.target.value))}
                className="w-full"
              />
              <div className="flex justify-between text-sm text-gray-400 mt-2">
                <span>{config.min} {config.unit}</span>
                <span className="text-yellow-300 font-bold">{params[config.id]} {config.unit}</span>
